...
// Helper function to extract URL from CDATA or plain text
function extractUrlFromElement(element) {
    ...
    if (content.includes('<!--[CDATA[') && content.includes(']]-->')) {
        // Extract URL from CDATA wrapper
...
export function wpSitemap (url) {
    ...
    response.html().find('sitemap loc').each(function (idx, el) {
        const sitemapUrl = extractUrlFromElement(el)
        const response = http.get(sitemapUrl)

        response.html().find('url loc').each(function (idx, el) {
            urls.push(extractUrlFromElement(el))
        })
    })
